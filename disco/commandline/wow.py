...
def main():
    ...
    parser.add_argument("--cell", nargs=6, default=None, type=float, help="Six floats corresponding to [a b c alpha beta gamma].")
    parser.add_argument("--dmin", default=None, type=float, help="Resolution cutoff in Angstroms.")
    parser.add_argument("--spacegroup", default=None, type=str, help="Space group.")
    ...
    for i in range(parser.n):
        if parser.spacegroup is not None:
            sg = gemmi.SpaceGroup(parser.spacegroup)
            cell = gemmi.UnitCell(*parser.cell)
            dmin = parser.dmin
        else:
            datum = data.sample()
            try:
        ...
        wavelength = np.reciprocal(np.sqrt((s1*s1).sum(-1)))
